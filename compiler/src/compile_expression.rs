...
impl<'a, 'ctx> compiler::Compiler<'a, 'ctx> {
    pub fn compile_expression(
        ...
        match expression {
            ...
            Expression::Call(function_call) => {
                ...
                let function_value_result =
                    self.compile_expression(function_call.left.deref().clone());
                ...
                match option_expr_value {
                    ...
                    Some(_) => {
                        ...
                        let compile_value = Value::from_compiled_value(
                            func_type,
                            basic_enum_value,
                            None,
                            false,
            ...
            Expression::Index(index_access) => {
                ...
                if compile_array_type.is_some() {
                    ...
                    // return type
                    ret_type =
                        compile_types::Type::ArrayTypeEnum(compile_array_type.clone().unwrap());
                }
                ...
                if compile_slice_type.is_some() {
                    ...
                    // return type
                    ret_type =
                        compile_types::Type::SliceTypeEnum(compile_slice_type.clone().unwrap());
                    ...
                    index_llvm_value = self.builder.build_int_truncate(
                        index_llvm_value,
                ...
                if length_known_at_compile_time {
                    ...
                    if index_llvm_value.is_const() {
                        let constant_index_value =
                            index_llvm_value.get_sign_extended_constant().unwrap();
                ...
                match variable {
                    ...
                    Err(_) => {
                        ...
                        let package_variable =
                            self.current_package.get_pkg_var(var_name.clone(), false);
            ...
            Expression::BasicLit(basic_literal) => match basic_literal.kind {
            ...
            Expression::CompositeLit(composite_lit) => {
                ...
                if compiled_composite_type.to_compile_array_type().is_some() {
                    ...
                    for keyed_element in composite_lit.val.values {
                        ...
                        match element {
                            ...
                            Element::LitValue(literal_value) => {
                                ...
                                let inner_ast_array_type = {
                                    match cloned_ast_type {
                                        Type::Array(array_type) => {
                                            Some(array_type.typ.deref().clone())
                                        }
                                        _ => None,
                                    }
                                }
                                ...
                                let wrapped_composite_lit =
                                    Expression::CompositeLit(ast::CompositeLit {
                                ...
                                let inner_ast_slice_type = {
                                    match cloned_ast_type {
                                        Type::Slice(slice_type) => {
                                            Some(slice_type.typ.deref().clone())
                                        }
                                        _ => None,
                                    }
                                }
                    ...
                    return self.compile_initial_slice_with_values(
                        compiled_composite_type,
                        compiled_items_values,
                    );
                } else {
    ...
    fn runtime_panic(&self, code_block: BasicBlock, message: String) {
        ...
        // call 'exit' function with value 1 to exit current process with exit code
