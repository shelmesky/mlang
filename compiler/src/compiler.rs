...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    fn do_compile(
        ...
        for dec in ast_file.decl {
            compiler.compile_declaration(&dec);
        }
        ...
        let filename_no_ext = Path::new(ast_file.path.as_path())
            .file_stem()
            .unwrap()
            .to_str()
            .unwrap();
    ...
    fn compile_declaration(&mut self, declaration: &ast::Declaration) {
        match declaration {
            ast::Declaration::Function(func_decl) => {
                ...
                // so we return i32 zero instead return nothing
                let compile_func_type = {
                    match ret_basic_type_enum {
                        None => {
                            void_return_function = true;

                            // let void function return a i32 value
                ...
                for (idx, arg) in func_val.get_param_iter().enumerate() {
                    ...
                    for arg_name in name_list_with_one_type {
                        ...
                        // get compile type of one argument
    ...
    pub fn convert_type(
        ...
        match ast_type {
            ...
            Type::Array(array_type) => {
                ...
                ArrayTypeEnum(compile_array_type)
            }
