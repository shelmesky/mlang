...
impl<'a, 'ctx> Compiler<'a, 'ctx> {
    pub fn compile_statement(&mut self, stmt: &parser::ast::Statement) {
        match stmt {
            Statement::Go(_) => {}
            ...
            Statement::Declaration(declaration) => match declaration {
                ...
                DeclStmt::Variable(declaration_list) => {
                    for variables in declaration_list.specs.iter() {
                        ...
                        let compile_variables_type = Compiler::convert_type(
                            self,
                        ...
                        if variables.values.len() > 1
                            && variables.values.len() != variables.name.len()
                        {
                        ...
                        if variables.values.len() == 1 {
                            ...
                            let compile_initial_value =
                                { self.compile_expression(initial_value).unwrap() };
                            ...
                            for (idx, variable) in variables.name.iter().enumerate() {
                                ...
                                if variable_values_option.is_some() {
                                    ...
                                    let compile_right_value =
                                        self.compile_expression(ast_right_value).unwrap();

                                    // store value to variable
                                    self.store_value(
                                        llvm_left_value,
                                        compile_right_value.llvm_value,
                                    );
                                }
